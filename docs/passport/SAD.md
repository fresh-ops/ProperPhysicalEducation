...
- **RAM:** 8 GB or more (as per non-functional requirements)
- **Ports:** Minimum 4 USB 3.0 ports (for EMG sensors and camera)
- **Peripherals:**
    - Standard RGB webcam (UVC protocol compatible)
    - Custom EMG sensors based on ESP32-3C microcontroller, connected via Bluetooth LE (BLE)

**Software Platform:**
- **Host OS:** Windows 10/11 64-bit (Primary) and Ubuntu 20.04 LTS
- **Development Language:** Python 3.10+ (both backend and frontend)

**Key Frameworks and Libraries:**

| Component | Technology | Purpose |
|-----------|------------|---------|
| **Computer Vision** | MediaPipe (Python wrapper) | Skeleton keypoint detection (33 landmarks) |
| **Camera Integration** | OpenCV (`cv2.VideoCapture`) | Cross-platform camera access and frame capture |
| **EMG Data Acquisition** | `bleak` | BLE communication with ESP32-3C sensors |
| **GUI Framework** | PySide6 | Main application window, real-time video rendering, reports display (Qt6 bindings with LGPL license) |
| **Data Processing** | NumPy | Efficient vector operations for angle calculations |
| **Data Structures** | Custom enums (`Landmark`, `Angle`) | Type-safe skeleton landmark definitions |
| **Pose Analysis** | Custom modules (`PoseDetector`, `calculate_deviations`) | Exercise matching and deviation analysis |
| **Serialization** | JSON (standard library) | Exercise definitions and pose descriptions |

**Core Architectural Components (from provided code):**

1. **Skeleton Representation:**
    - `Landmark` enum defines 33 MediaPipe-compatible keypoints with 3D coordinates [x, y, z].
    - `Angle` enum defines biomechanical joints (shoulder, elbow, knee) as triplets of landmarks.

2. **Pose Analysis Pipeline:**
    - `calculate_angle_xy()`: Computes 2D joint angles (XY plane projection) using vector mathematics, with handling for degenerate cases (zero-length vectors).
    - `landmarks_to_pose()`: Converts raw landmark arrays into structured `Pose` objects with 6 key angles (left/right shoulder, elbow, knee).
    - `PoseDetector`: Matches current pose against reference poses from exercise database.
    - `calculate_deviations()`: Computes per-joint angle deviations for corrective feedback generation.

**Data Storage:**
- **Exercise Database:** JSON files stored in file system, containing reference poses with name, threshold values, and expected angle ranges.
- **User Progress & Reports:** SQLite database for local storage of session history, calibration data, and generated reports.
- **EMG Calibration Data:** SQLite database tables for individual user calibration profiles (preferred) or binary/CSV files as alternative.

**Communication Protocols:**
- **Camera:** Standard camera drivers accessed via OpenCV
- **EMG Sensors:** BLE GATT protocol using `bleak` library, data streamed as serial-like values over Bluetooth
- **Internal Module Communication:** Direct Python function calls (synchronous) between analysis modules

**Deployment Model:**
Standalone desktop application running entirely on user's local machine. No cloud dependencies required, ensuring privacy and low-latency feedback (<150 ms requirement).
...
@gdclass
class skeletal_restorer(Node):
	"""
	A SkeletalRestorer class is an agregator of skeletal landmarks that restores the whole digital
	skeleton averaging the passed points.
	"""
	def init(self):
		"""
		Initialize SkeletalRestorer. Resets the base CameraFeed and the translation table. Can be
		used as a restore function to reuse one instance of this class.
		
		Returns:
			SkeletalRestorer: This initialized node.
		"""
		self.__base_feed_id: int = -1
	...
	def add_camera(self, camera_feed_id: int, base: Array, translating: Array) -> None:
		...
		Args:
			...
			base (py4godot.classes.core.Array): The set of points in the basis of the base 
				CameraFeed. Should be a 2d n-by-m matrix. Every element should be a float value
			translating (py4godot.classes.core.Array): The set of points in the basis of a new 
				CameraFeed. Should have the same shape as a base. Every its point should correspond
				to the basee one. Every element should be a float value
		"""
		base = array_to_ndarray(base, 2).astype(np.float64)
		translating = array_to_ndarray(translating, 2).astype(np.float64)

		self.__translators[camera_feed_id] = BasisTranslator(base[:, :3], translating[:, :3])

	def restore(self, landmarks: Dictionary) -> Array:
		"""
		Restores the full digital skeleton using the passed landmarks.

		Args:
			landmarks (py4godot.classes.core.Dictionary): A dictionary with CameraFeed ids as a
				keys and godot Array matrices as a values. Every key CameraFeed id should have a
				translator added with add_camera() method. Every matrix should have the same shape:
				n points of format [x, y, z, visibility, presence]. Every value of a point should
				be float

		Returns:
			py4godot.classes.core.Array: A set of digital skeleton points. If the base feed is not
			set, the returned array is empty
		"""
		if self.__base_feed_id == -1:
			return Array.from_list([])
		all_points = []
		for key in landmarks.keys():
			if key not in self.__translators:
				self.add_camera(key, landmarks[self.__base_feed_id], landmarks[key])
	...
	def set_base_feed_id(self, feed_id: int):
		self.__base_feed_id = feed_id

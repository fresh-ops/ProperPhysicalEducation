...
const int POLLING_RATE = 10; // refresh speed      
...
const char* mqtt_server = "10.73.103.71"; // IP of main device
...
void loop() {
  ...
  if (now - lastSend >= interval) {
    ...
    // 2 bytes for sending
    uint8_t payload[2];
    payload[0] = sensorValue & 0xFF;        // little byte
    payload[1] = sensorValue >> 8;   

    // QoS=0
    client.publish(MQTT_MAIN_TOPIC, payload, 2, false);
  }
}

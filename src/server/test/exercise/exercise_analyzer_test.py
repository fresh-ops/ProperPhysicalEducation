...
exercise_loader = ExerciseLoader(
    PoseLoader(directory_path="data/pose"), directory_path="data/exercise"
)
lateral_raises_exercise = exercise_loader.load_exercise(1)

test_data = [
    (lateral_raises_exercise, [arms_down_pose, t_pose, arms_down_pose], [0, 0, 0]),
    (
        lateral_raises_exercise,
        [arms_down_pose, incorrect_t_pose, arms_down_pose],
        [0, 1, 1],
    ),
    (lateral_raises_exercise, [t_pose, arms_down_pose, t_pose], [1, 0, 0]),
]


@pytest.mark.parametrize("exercise, pose_sequence, expected_results", test_data)
def test_analyze(
    exercise: Exercise, pose_sequence: list[Pose], expected_results: list[int]
) -> None:
